...
public class P6eCrossDomainFilter implements Filter {
     ...
     * 自定义的请求头用来处理项目的多次跨越处理的问题
     *
     * 如何处理多次跨越的呢？
     * 我们知道请求的跨域处理是在请求的返回头中添加跨域处理的数据
     * 请求是过来的时候是不知道当前请求是否在前面或者后面进行跨域处理
     *
     * 我的解决方案是，在请求的请求头中添加相应的标记数据
     * 例如：我在请求头中添加 cross-domain 如果值为 existence 表示跨域处理过了不需要再跨域处理了
     *      如果不存在请求头 cross-domain 或者值不为 existence 表示没有跨域处理，现在需要跨域处理
     *
     * @author lidashuang
     * @version 1.0
     */
