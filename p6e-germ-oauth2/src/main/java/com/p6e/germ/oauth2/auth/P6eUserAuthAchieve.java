...
@Order(100)
@Component
public class P6eUserAuthAchieve {
    ...
    /** 注入日志系统 */
    private static final Logger logger = LoggerFactory.getLogger(P6eUserAuthAchieve.class);
    ...
    private Object around(final ProceedingJoinPoint pjp) throws Throwable {
        // 读取方法上面的注解对象
        final P6eUserAuth p6eUserAuth = P6eAuthTool.getMethodAnnotation(pjp, P6eUserAuth.class);
        if (p6eUserAuth != null) {
            // 获取身份凭证
            ...
            if (token == null) {
                ...
                if (p6eTokenResultDto == null || p6eTokenResultDto.getError() != null) {
                    ...
                    if (args != null) {
                        for (Object arg : args) {
                            if (arg instanceof P6eUserAuthModel) {
                                P6eUserAuthModel model = (P6eUserAuthModel) arg;
                                model.setId(0);
                                model.setStatus(0);
                                model.setAvatar("");
                                model.setName("");
