...
public class P6eJurisdictionAspectModel implements Serializable {
    ...
    public boolean isExist(String content) {
        for (final Jurisdiction jurisdiction : jurisdictionData) {
            if (jurisdiction.isExist(content)) {
                return true;
            }
        }
        return false;
    ...
    public static class Jurisdiction {
        ...
        public boolean isExist(String content) {
            ...
            if (index > -1) {
                return isExist(content.substring(0, index), content.substring(index + 1));
            }
            return false;
        }

        public boolean isExist(String name, String param) {
            return this.name.equals(name)
                    && this.param.equals(param)
                    && P6eJsonUtil.fromJsonToMap(this.getContent(), String.class, String.class).get(param) != null;
        }

        public boolean isExist(Jurisdiction jurisdiction) {
